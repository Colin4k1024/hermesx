...
|------------|----------------|---------------|----------|-----------|
| Multi-tenancy | Done | Released baseline | PostgreSQL RLS migrations, tenant middleware, tenant-scoped stores, integration tests under `tests/integration/`, [2026-06-09 release evidence](artifacts/2026-06-09-enterprise-release-evidence/README.md) | Keep live PostgreSQL RLS tests in CI; tenant SQL static gates must keep passing |
| Auth, API keys, RBAC | Done | Released baseline plus OIDC plan | `internal/auth/`, `internal/middleware/rbac.go`, API key scopes, `RBAC_MATRIX.md`, [OIDC integration test plan](runbooks/enterprise-OIDC-integration-test-plan.md), [local OIDC harness evidence](artifacts/2026-06-09-enterprise-release-evidence/README.md) | Attach Keycloak/Auth0 live IdP E2E evidence before enterprise sign-off |
...
| Egress control | Done | Current branch `v2.4.0-dev` | `SecureTransport`, tenant allowlist rules, production `deny-all` default, `HERMES_EGRESS_DEFAULT` override, persisted `egress.denied` audit events surfaced by `/admin/v1/egress/blocked-log` | Add live allowlist smoke test in production-like deployment |
...
| Observability | Done | Released baseline plus Unreleased pre-built pack | Prometheus metrics, OTel tracing, structured logging, [Grafana dashboard](../deploy/grafana/dashboards/hermesx-overview.json), [alerts](../deploy/prometheus/alerts.yml), [Prometheus config](../deploy/prometheus/prometheus.yml), Helm/Compose render evidence | Import dashboard and dry-run alerts in staging |
| Backup and disaster recovery | Partial | Released PG backup baseline plus Unreleased Redis/MinIO scripts | PG backup/restore docs, [scripts/dr-test.sh](../scripts/dr-test.sh), [scripts/pitr-drill.sh](../scripts/pitr-drill.sh), [PITR runbook](runbooks/pg-pitr-recovery.md), [2026-06-09 local evidence](artifacts/2026-06-09-enterprise-release-evidence/README.md) | Local DR did not pass because backup directories/PITR container were absent; record RTO/RPO in a prepared environment before sign-off |
...
| CI and security gates | Done | Released baseline plus current-branch supply-chain artifacts | Go tests, integration tests, race/coverage workflows, govulncheck, gosec, Trivy, CodeQL, Dependabot, CycloneDX SBOM workflow, release provenance attestation | Add DAST/container runtime checks and image signing if production policy requires them |
...
| OIDC E2E is not yet recorded against external IdPs | Medium | Local OIDC harness passes; run [enterprise-OIDC-integration-test-plan.md](runbooks/enterprise-OIDC-integration-test-plan.md) and attach Keycloak/Auth0 evidence before enterprise sign-off |
...
| Backup/DR is uneven across data stores | Medium | 2026-06-09 local DR check did not pass because backups and PITR container were unavailable; keep Redis/MinIO scripts Unreleased until automated restore drills run |
...
| Supply-chain signing is partial | Medium | SBOM and release provenance are generated; container/image signing is still a follow-up for environments that require cosign policy enforcement |
